        ...
        if($updateIps == 'enabled')
        {
            ...
            foreach ($ipsDomainsMaps as $map) 
            {
                if(key_exists('ip',$map) && count($map['ip']) == 2 && key_exists('domain',$map) && count($map['domain']) == 2)
                {
                    ...
                    $ipObject->setCreated_at(date("m-d-Y"));
                    $ipObject->setLast_updated_by(intval(Arrays::getElement($user,'id')));
                    $ipObject->setLast_updated_at(date("m-d-Y")); 
                ...
                if($updateRecords == 'enabled')
                {
                    ...
                    if($useSubDomains == 'enabled')
                    {
                        ...
                        $dnsRecords = array(
                            ...
                            "HostName9" => "@",
                            "RecordType9" => "TXT",
                            "Address9" => "v=spf1 ip4:{$mainIp['value']} -all",
                            "TTL9" => "60"
                        );
            ...
            foreach ($ips as $row) 
            {
               if($row['rdns'] != '')
               {
                   $domain = count(explode('.',$row['rdns'])) > 2 ? Domains::getDomainFromURL($row['rdns']) : $row['rdns'];
                    $vmta = str_replace(array('$P{IP}','$P{DOMAIN}'),array($row['value'],$domain),file_get_contents($assetsDirectory . DS . DEFAULT_TEMPLATES_DIRECTORY . DS . 'installation' . DS . 'vmta.tpl')) . PHP_EOL;
